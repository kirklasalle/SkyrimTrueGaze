...
$htmlCandidates = @(
    (Join-Path $projectRoot 'TrueGazeConfig.html'),
    (Join-Path (Split-Path -Parent $projectRoot) 'TrueGazeConfig.html'),
    (Join-Path $scriptDir 'TrueGazeConfig.html')
)
$htmlPath = $htmlCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $htmlPath) { $htmlPath = Join-Path $projectRoot 'TrueGazeConfig.html' }
