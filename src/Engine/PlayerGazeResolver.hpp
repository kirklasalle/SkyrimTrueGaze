...
#include "PCH.h"
#include "Bridge/TelemetryPacket.h"
#include <cstdint>
...
namespace TrueGaze::Engine
{
    ...
    {
    public:
        /// @brief Latest confidence-qualified HCEP gaze observation.
        struct HcepSignal
        {
            ...
            /// Phase S4: full human-intent context. The gaze vector above is the
            /// fused eye+head world direction; these fields carry the raw sensor
            /// context so diagnostics can explain WHY a fusion result occurred.
            float headYawRad{0.0f};
            float headPitchRad{0.0f};
            float headRollRad{0.0f};
            float convergenceMeters{0.0f};
            uint8_t blinkBitmask{0};
            uint64_t timestampUs{0};
            uint64_t receivedSteadyMs{0};
        };

        /// @brief Snapshot of the current HCEP intent state for diagnostics.
        struct IntentDiagnostic
        {
            ...
            float convergenceMeters{0.0f};
            bool convergencePlausible{false};
        };
        ...
        /// @brief Publishes the latest validated HCEP observation for this game frame.
